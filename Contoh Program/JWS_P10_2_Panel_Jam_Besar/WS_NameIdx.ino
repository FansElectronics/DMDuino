...
char* msgPuasa(int hd, int ty) // get sholat name from EEPROM
{
  ...
  char  tyBuff[13];
  locLen = (ty - 1) * 13;
  memccpy_P(tyBuff, TY_Puasa + locLen  , 0, 11);
  if (hd == 1)
  {
...
char * drawDayDate()
{
  ...
  //sprintf(out, "%s, %d %s %d", DayName(daynow), now.day(), MonthName(now.month()), now.year());
  sprintf(out,"%s %s, %02d-%02d-%04d   %d %s %dH\0",DayName(daynow),pasar[jumlahhari()%5],now.day(),now.month(),now.year(),nowH.hD,locBuff,nowH.hY);
  return out;
}
